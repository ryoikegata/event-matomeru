...
export const useUserEventAttend = (event_id: number, userId: string | undefined) => {
  const [userEventAttend, setUserEventAttend] = useState<boolean | null>(null);
  
  useEffect(() => {
    const fetchUserEventAttend = async () => {
    try {
    const { data: userEventAttend, error: userEventAttendError } = await supabase
      ...
      if (userEventAttendError) {
        console.error('Error fetching user event attend status:', userEventAttendError);
